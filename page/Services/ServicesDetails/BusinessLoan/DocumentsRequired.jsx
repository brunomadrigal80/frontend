...
import { FileText, CreditCard, Landmark, FileSpreadsheet, BriefcaseBusiness } from "lucide-react";
...
const DocumentsRequired = () => {
  const docs = [
    ...
    { title: "Business Registration", icon: <Landmark className="w-6 h-6" /> },
    { title: "Bank Statements", desc: "(6–12 months)", icon: <BriefcaseBusiness className="w-6 h-6" /> },
    { title: "Income Proof / ITR", icon: <FileSpreadsheet className="w-6 h-6" /> },
  ];
  ...
  return (
    <section className="py-24 bg-slate-50 relative overflow-hidden">
      ...
      <div className="max-w-6xl mx-auto px-6 text-center relative z-10">
        ...
        >
          <div className="inline-block mb-3 px-4 py-1.5 bg-orange-100/50 border border-orange-200 rounded-full text-orange-700 text-sm font-bold tracking-wide shadow-sm">
            DOCUMENTATION
          </div>
          ...
          <p className="text-gray-500 max-w-2xl mx-auto mb-16 text-lg">
            Ensure an instantaneous disbursement by keeping these 5 critical business documents ready.
          </p>
        </motion.div>
        ...
        >
          {docs.map((doc, index) => (
            ...
            >
              <div className="w-14 h-14 rounded-full bg-orange-50 text-orange-600 flex items-center justify-center mb-4 group-hover:scale-110 group-hover:bg-orange-600 group-hover:text-white transition-all duration-300 shadow-sm">
                {doc.icon}
              </div>
              ...
              {doc.desc && (
                <span className="text-xs text-gray-400 mt-1">{doc.desc}</span>
              )}
            </motion.div>
          ))}
        </motion.div>
